...
// 判斷通知類型
function noticeType(type, name, options) {
    switch (type) {
        case 1:
            return `您有新會議：「${name}」。`
        case 2:
            return `會議「${name}」附件已上傳，請點擊查閱。`
    }
}


module.exports = { getAttendType, getAnotherType, getAttendMode, getSign, equal, allowChairSign, allowRoomSign, allowEdit, allowEditOther, allowChooseMode, noticeType }